...
this uses a "Cohort Tracking" pattern. It holds the fitted FactorModel for each 
entry date in memory and vectorizes the mark-to-market calculations for all 
active cohorts in a single pass per day.
"""
...
class Backtest:
    ...
    def __init__(
        ...
        # Determine the sequence of valid trading days based on the data index
    ...
    def run(self) -> BacktestResults:
        ...
        # Dictionary of active trades keyed by entry_date.
        ...
        for current_date in self._walk_forward_dates:
            ...
            for entry_date in list(active_cohorts.keys()):
                ...
                # Why: We want to measure the correction of the alpha signal (actual - expected), 
                # isolating the trade from broader market beta.
                if self.config.return_type in {"residual", "both"}:
                    scores = model.score(today_etfs, target_returns)
                    # model.score() defines delta as (expected - actual). 
                    # We invert it to get realized outperformance (actual - expected).
            ...
            # ---------------------------------------------------------
            # Slice the trailing N days (exclusive of today) to prevent look-ahead bias
        ...
        # Add this guard block to ensure columns always exist!
        if ledger_df.empty:
